# Citadel Citadel Governance Persistence

## Scope

Citadel Citadel Governance owns ref-only Citadel authority, governance, or observability persistence evidence owner. This document is package-local and is the persistence contract for `core/citadel_governance` in `citadel`.
...
```bash
cd core/citadel_governance && mix test; root mix ci
```
