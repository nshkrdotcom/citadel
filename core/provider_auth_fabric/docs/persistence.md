# Citadel Provider Auth Fabric Persistence

## Scope

Citadel Provider Auth Fabric owns ref-only Citadel authority, governance, or observability persistence evidence owner. This document is package-local and is the persistence contract for `core/provider_auth_fabric` in `citadel`.
...
```bash
cd core/provider_auth_fabric && mix test; root mix ci
```
