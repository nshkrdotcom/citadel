# Citadel Connector Binding Persistence

## Scope

Citadel Connector Binding owns ref-only Citadel authority, governance, or observability persistence evidence owner. This document is package-local and is the persistence contract for `core/connector_binding` in `citadel`.
...
```bash
cd core/connector_binding && mix test; root mix ci
```
