# Citadel Native Auth Assertion Persistence

## Scope

Citadel Native Auth Assertion owns ref-only Citadel authority, governance, or observability persistence evidence owner. This document is package-local and is the persistence contract for `core/native_auth_assertion` in `citadel`.
...
```bash
cd core/native_auth_assertion && mix test; root mix ci
```
