# Citadel Authority Contract Persistence

## Scope

Citadel Authority Contract owns ref-only Citadel authority, governance, or observability persistence evidence owner. This document is package-local and is the persistence contract for `core/authority_contract` in `citadel`.
...
```bash
cd core/authority_contract && mix test; root mix ci
```
