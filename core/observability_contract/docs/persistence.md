# Citadel Observability Contract Persistence

## Scope

Citadel Observability Contract owns ref-only Citadel authority, governance, or observability persistence evidence owner. This document is package-local and is the persistence contract for `core/observability_contract` in `citadel`.
...
```bash
cd core/observability_contract && mix test; root mix ci
```
