# Citadel Jido Integration Contracts Persistence

## Scope

Citadel Jido Integration Contracts owns ref-only Citadel authority, governance, or observability persistence evidence owner. This document is package-local and is the persistence contract for `core/jido_integration_contracts` in `citadel`.
...
```bash
cd core/jido_integration_contracts && mix test; root mix ci
```
...
Valid durability claims require explicit profile selection, adapter capability, migration or substrate preflight, redacted evidence, focused tests, repo QC, and a pushed commit. Durable profiles add storage refs and receipt refs only; they do not authorize raw auth, provider, or Brain policy payload persistence.
...
No raw durable schema owned here unless a future adapter package documents migrations and preflight.
